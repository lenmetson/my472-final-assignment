---
title: "What, if any, characteristics and factors discriminate MPs who tend to ask questions about economic issues from MPs who tend to ask questions about health and welfare issues?" 
subtitle: "MY472 Summative 3 Project"
...
# Please note, all data collection chunks are set to eval=FALSE. 
# They only need to be run once to populate the local relational database.
# To knit this document, chang eval = FALSE to eval = TRUE. 
# Please note, this will take a long time to run because of the scraping code
...
replace_null_with_na <- function(x) { # NOTE function adapted from ChatGPT output
...
# When flattening the API responses, we replace null values with character "NA" values (using the function above), this function converts them back to NAs that R can recognise: 
...
# Connect to local relational database
...
The purpose of parliamentary questions are for MPs to represent their constituents’ interests. In this project, I focus on asking whether demographic factors of an MP’s constituency discriminates whether they tend to ask economic or health & welfare questions.

# Data

I drew on data from two sources: (1) the UK Parliament API (“API”), and (2) the [UK House of Commons constituency dashboard](https://commonslibrary.parliament.uk/constituency-dashboard/) (“dashboard”). I limit my analysis to questions asked in 2023. 

First, I pulled the text of questions from the API oral and written question endpoints. The API only returns oral questions asked in the House of Commons (HoC). However, it returns written questions from both the House of Lords (HoL) and the HoC. Therefore, when requesting the written question endpoint, I filtered questions to ones from the HoC. I merged oral and written questions into one table: `questions`. 

For each question, I wanted to be able to pull additional information about the MP. Some MP characteristics – e.g. their party - change over time. Thus, I queried the API for each unique MP-date pair from my `questions` table.

However, I did not want to write out a table with data on each MP for every day they had asked a question. Therefore, I grouped the clean response table by each unique combination of MP and their characteristics and summarised the earliest and latest date this combination was valid for. I used this range to select the correct row for each question in later SQL queries.

```{r pull-oral-questions}

# This code can also be run by sourcing scripts/01_pull-oral-questions.R 

# Define function to construct request URL and execute request
GET_qs <- function(endpoint_url, n_skip = 0) {
  ...
  response <-
    httr::GET(url) %>%
    httr::content("parsed") # Use :: because the tm package masks content 

  return(response)
}

# Define function to pull all oral questions
...
pull_all_oral_qs <- function(endpoint_url){
  ...
  # calculate how many times we will have to pull
...
## Apply pull oral question function 
...
# Define function to pull all written questions 
...
oral_question_df <- oral_question_df %>%
  # Select a subset of variables to keep
  select(
    question_id = Id,
...
for (i in seq_along(written_questions)) { 
  ...
  # Replace nulls with NAs so lengths of lists are constant
...
# As MP characteristics - e.g. party affiliation - change over time, we need to 
# ensure the information we pull is valid for the day of the question.
# The approach I used was to make a request for each unique day-MP pairs. 
# This can later be summarised into ranges for which any unique set of MP covariates are valid.
...
#Define a list of unique constituencies we want to pull information for 
...
I collected further data on the electoral histories of MP’s constituencies from the API, however, demographic data was limited. Therefore, I used the HoC Library constituency dashboard to add demographic variables. This data source does not have an API endpoint and requires each constituency to be looked up using a search tool. Therefore, I used Selenium to interactively scrape the dashboard. I merged data from these two sources and wrote them to the table `constituencies`.
...
# Change age variables to numbers
...
# Change universal credit variable to numbers

cons_hoc$uc_claimants_hoclib23 <- cons_hoc$uc_claimants_hoclib23 %>%
  str_remove_all(",") %>%
  as.numeric()

# Change house price variable to numbers
...
# Pull party information to get party names and abbreviations. 
# We use this when matching an MPs current party with previous election results. 
# Party colours could also be used to colour plots with accruate colours. 
...
To measure whether a question is about (1) economic issues or (2) health and welfare, I used a simple dictionary approach. Whilst limited compared to machine learning classification approaches, dictionary string matching was more feasible for this project. I used pre-defined policy topic dictionaries from [Lexicoder](https://www.snsoroka.com/data-lexicoder) (Albugh, et al., 2013). I wrote out the results of my measurement to the table `question_topics`. 
...
# Download Lexicon Policy Topic Dictionaries # NOTE Citation: Albugh, Quinn, Julie Sevenans and Stuart Soroka. 2013. Lexicoder Topic Dictionaries, June 2013 versions, McGill University, Montreal, Canada.

if (!file.exists("data/lexicoder_dictionaries/LTDjun2013/policy_agendas_english.lcd")) { # Check whether the file has already been downloaded
...
# These files are stored in a propeitrary file format .lcd. However, they are actually XML files. 
# Therefore, we can parse them using R's XML package if we read the file in as a raw string 
# then parse it. 
# After parsing we can conver the XML file into a list of dictionaries. 
...
# Select relevant subdictionaries and make terms into a single list

econ_dict <- c( # Select relevant dictionaries.  
  dictionaries$macroeconomics, # Contains 84 terms 
  dictionaries$finance, # Contains 54 terms 
  dictionaries$foreign_trade # Contains 29 terms
  )

health_welf_dict <- c( # Select relevant dictionaries 
  dictionaries$healthcare, # Contains 128 terms
  dictionaries$social_welfare # Contains 23 terms 
)
...
# Use str_detect to detect any occurances of the strings in the dictionary 
...
A crows foot diagram of the final local relational database is shown below: 

![](images/my472-crow-foot.svg)
...
To answer which factors about an MP’s constituency discriminate whether they ask questions on economic or health & welfare topics, I constructed a measure of “slant” at the MP level, defined as: 

$$ 
slant = \frac{N (economic)}{N(questions)} - \frac{N(health\_welfare)}{N(questions)}  
$$

This measure ranges from -1 to 1. A value of -1 indicates a perfect health & welfare slant – I.e. the MP only asks health & welfare questions - and a value of 1 indicates a perfect economic slant. 

```{r read-in-analysis-data, eval = TRUE}

# Query data base to construct analysis data
# Once the database has been populated once, all analysis code can be run from here. 
# This is where the Rmarkdown file knits from 
...
analysis_df <- dbGetQuery(
  ...
  LEFT JOIN members ON questions.asking_member = members.member_id
    ...
    AND REPLACE(questions.question_tabled_when, '-', '') 
      BETWEEN REPLACE(members.member_date_valid_min, '-', '') /* no date class in SQLite, so convert to string*/
...
# Calculate slant variable 

analysis_df <- analysis_df %>%
  mutate(econ_slant = econ_prop - health_welf_prop)
```

```{r hist, eval = TRUE, fig.cap="Distribution of slant variable for oral and written questions asked by MPs in 2023. Black line indicates where there is no slant."}
...
# Add axis labels # NOTE: based on https://stackoverflow.com/questions/12409960/ggplot2-annotate-outside-of-plot and https://stackoverflow.com/questions/10836843/ggplot2-plot-area-margins

hist +  
  ggplot2::annotate("segment", y =  -20 , yend = -20, x = -0.15, xend = -0.95, arrow = arrow()) + 
  ggplot2::annotate("segment", y =  -20 , yend =  -20, x = 0.15, xend = 0.95, arrow = arrow()) + 
  ggplot2::annotate("text", y = -25, x = -0.5, label = "More health & welfare", size = 3) +
  ggplot2::annotate("text", y = -25, x = 0.5, label = "More economic", size = 3) +
  coord_cartesian(xlim = c(-1,1),  ylim=c(0, 150), clip="off") +
  theme(plot.margin = unit(c(1,1,4,1), "lines"))

```

Figure 1 shows that most MPs have no slant but there is a slight skew towards health & welfare questions. Breaking this down by the two main parties, Figure 2 shows that Conservative MPs still slightly skewed towards health & welfare, but much less than Labour MPs. 

```{r hist-party, eval = TRUE, fig.cap="Distribution of slant variable for oral and written questions asked by MPs in 2023 amongst MPs in the two main parties. Black line indicates where there is no slant."}

hist_con <- analysis_df %>%
  filter(party_abbreviation == "Con")%>%
  ggplot() +
  geom_vline(xintercept = 0)+
  geom_histogram(aes(x=econ_slant), alpha = 0.7, fill = "#0000ff") +
  xlim(-1,1)+
  labs(
    title = "Conservative MPs", 
    x = "Slant", y = "Count") +
  theme(
    aspect.ratio = 1,
    panel.background = element_rect(fill = "white", color = "black"), 
  ...
  ggplot2::annotate("text", y = -25, x = -0.5, label = "More health & welfare", size = 3) +
  ggplot2::annotate("text", y = -25, x = 0.5, label = "More economic", size = 3) +
  coord_cartesian(xlim = c(-1,1),  ylim=c(0, 80), clip="off") +
  theme(plot.margin = unit(c(1,1,4,1), "lines"))


hist_lab <- analysis_df %>%
  filter(party_abbreviation == "Lab")%>%
  ggplot() +
  geom_vline(xintercept = 0)+
  geom_histogram(aes(x=econ_slant), alpha = 0.7, fill = "#ff0000") +
  xlim(-1,1)+
  labs(
    title = "Labour MPs", 
    x = "Slant", y = "Count") +
...
grid.arrange(hist_con, hist_lab, nrow=1)

```

```{r prepare-map, eval = TRUE, include = FALSE}
...
httr::GET("https://api.os.uk/downloads/v1/products/BoundaryLine/downloads?area=GB&format=ESRI%C2%AE+Shapefile&redirect", write_disk("data/whole_UK_shapefile/OS_zip.zip", overwrite = TRUE))

options(timeout=60) # Reset timeout

# unzip # FIXME not working 
unzip(
...
Figure 3 shows the geographic distribution of question slant. Whilst there is no clear regional pattern, it seems that urban areas, such as the North of Wales and North West of England appear to be slightly more focused on health & welfare. Thus, MPs’ slant is potentially correlated with urbaneness of their constituency.

```{r plot-map, eval = TRUE, fig.width = 40, fig.height =60, fig.cap= "Map showing the geographic distribution of question slant. A slant score of -1 indicates all questions were about health and welfare issues, a score of 1 indicates all questions were about economic issues. Constituencies shown as grey are where the MP asked no oral or written questions in 2023."}
...
map <- 
  ...
  tm_layout(
    main.title = "Distribution of question topic slant across UK Constituencies", 
    main.title.size = 5,
...
Exploring this, I correlate population density with slant. Figure 4 shows population density is correlated with having a stronger focus on health & welfare compared to economic issues. However, urban places may tend to be represented by Labour MPs, or may be more electorally competitive. Therefore, I add controls for party and seat marginality (defined as the mean majority an MP’s party won in the previous four elections). Adding these controls eliminates the correlation.  

```{r pop-density, eval = TRUE, fig.cap = "A slant score of -1 indicates all questions were about health and welfare issues, a score of 1 indicates all questions were about economic issues."}

lm_density <- summary(lm(econ_slant ~ density + party + mean_marginality, data = analysis_df))
lm_density_biv <- summary(lm(econ_slant ~ density, data = analysis_df))

slope_density <- lm_density$coefficients[[2,1]]
se_density <- lm_density$coefficients[[2,2]]

slope_density_biv <- lm_density_biv$coefficients[[2,1]]
se_density_biv <- lm_density_biv$coefficients[[2,2]]
...
plot_density <- analysis_df %>%
  ...
  labs(
    title = "Correlation between population density of an MP's consitutency\nand their question slant in 2023", 
...
plot_density +  
  geom_text(
    aes(x=20000, y= slope_density*20000), 
    label = paste0(
      "Bivariate:",
      "\nSlope: ", signif(slope_density_biv, digits = 3),
      "\nSE: ", signif(se_density_biv, digits = 3), 
      "\n\nWith controls:",
      "\nSlope: ", signif(slope_density, digits = 3),
      "\nSE: ", signif(se_density, digits = 3) 
    ), size = 3)+ 
  ggplot2::annotate("segment", x =  -3000 , xend = -3000, y = -0.15, yend = -0.9, arrow = arrow()) + 
  ggplot2::annotate("segment", x =  -3000 , xend =  -3000, y = 0.15, yend = 0.9, arrow = arrow()) + 
  ggplot2::annotate("text", x = -3500, y = -0.5, label = "More health & welfare", size = 3, angle = 90) +
   ggplot2::annotate("text", x = -3500, y = 0.5, label = "More economic", size = 3, angle = 90) +
  
  coord_cartesian(xlim = c(0,17000),  ylim=c(-1, 1), clip="off") +
  theme(plot.margin = unit(c(1,6,1,3), "lines"))
```

Finally, the relationship between health & welfare questions and population could be driven by more welfare recipients living in urban areas. Figure 5, however, shows there is no overall relationship – whether controls are added or not - between the number of households receiving universal claimants and focus on health & welfare issues. 

```{r con-uc-claimants, eval = TRUE, fig.cap = "A slant score of -1 indicates all questions were about health and welfare issues, a score of 1 indicates all questions were about economic issues."}

# Run linear model
lm_uc <- summary(lm(econ_slant ~ uc_claimants + party + mean_marginality, data = analysis_df))
lm_uc_biv <- summary(lm(econ_slant ~ uc_claimants, data = analysis_df))
# Save slope coefficients and SEs for plotting

slope_uc <- lm_uc$coefficients[[2,1]]
se_uc <- lm_uc$coefficients[[2,2]]

slope_uc_biv <- lm_uc_biv$coefficients[[2,1]]
se_uc_biv <- lm_uc_biv$coefficients[[2,2]]
...
plot_uc <- analysis_df %>%
  ...
  geom_smooth(method = "lm") +
  labs(
    title = "Correlation between the number of households claiming\nUniversal Credit in an MP's consitutency and their question\nslant in 2023", 
    x = "Number of households claiming Universal Credit in 2023", 
    y = "Slant\n\n") +  
  ylim(-1,1) +
  theme(
    aspect.ratio = 1,
...
plot_uc +  
  geom_text(
    aes(x=26000, y= slope_uc*26000), 
    label = paste0(
      "Bivariate:",
      "\nSlope: ", signif(slope_uc_biv, digits = 3),
      "\nSE: ", signif(se_uc_biv, digits = 3), 
      "\n\nWith controls:",
      "\nSlope: ", signif(slope_uc, digits = 3),
      "\nSE: ", signif(se_uc, digits = 3) 
    ), size = 3)+ 
  ggplot2::annotate("segment", x =  -2700 , xend = -2700, y = -0.15, yend = -0.9, arrow = arrow()) + 
  ggplot2::annotate("segment", x =  -2700 , xend =  -2700, y = 0.15, yend = 0.9, arrow = arrow()) + 
  ggplot2::annotate("text", x = -3300, y = -0.5, label = "More health & welfare", size = 3, angle = 90) +
  ggplot2::annotate("text", x = -3300, y = 0.5, label = "More economic", size = 3, angle = 90) +
  
  coord_cartesian(xlim = c(1000,22000),  ylim=c(-1, 1), clip="off") +
  theme(plot.margin = unit(c(1,6,1,3), "lines"))

```

Overall, controlling for an MP’s party and marginality, the urbaneness of an MP's seat does not appear to predict question slant. This suggests that other factors about an MP’s seat, or factors related to the MP themselves – e.g. their party -  may be better discriminators of whether they ask economic or health & welfare questions.
