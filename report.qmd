...
analysis_df <- dbGetQuery(
  ...
  SELECT 
    ...
    constituencies.uc_claimants_hoclib23 / constituencies.population_hoclib23 AS uc_claimants,
...
```{r plot-map, eval = TRUE, fig.width = 40, fig.height =60, fig.align = 'center'}
# Plot bias
...
plot_age_30_64 <- analysis_df %>%
  ...
  geom_text(
    aes(x=0.5, y= 0.5),     label = paste0(
      "Slope: ", signif(slope_age_30_64, digits = 3),
      "\n SE: ", signif(se_age_30_64, digits = 3)
    ),
    size = 3)+  
  ylim(-1,1) +
  theme(
...
plot_age_65 <- analysis_df %>%
  ...
  geom_text(
    aes(x=0.3, y= 0.5),     
    label = paste0(
      "Slope: ", signif(slope_age_65, digits = 3),
      "\n SE: ", signif(se_age_65, digits = 3)
    ),
    size = 3)+  
  ylim(-1,1) +
  theme(
