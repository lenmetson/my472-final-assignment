...
# Run linear model 
lm_marg <- summary(lm(health_welf_bias ~ mean_marginality, data = analysis_df))
...
plot_marginality <- analysis_df %>%
  ggplot(aes(x=mean_marginality, y = health_welf_bias)) + 
...
# Run linear model
lm_uc <- summary(lm(health_welf_bias ~ uc_claimants, data = analysis_df))
...
plot_uc <- analysis_df %>%
  ggplot(aes(x=uc_claimants, y = health_welf_bias)) + 
  geom_point(size = 0.5, alpha = 0.5) + 
  geom_smooth(method = "lm") +
  geom_text(
    aes(x=-0.2, y= 0.5), 
...
# Run linear models 
lm_house_price <- summary(lm(health_welf_bias ~ median_house_price, data = analysis_df))
...
plot_house_price <- analysis_df %>%
  ggplot(aes(x=median_house_price, y = health_welf_bias)) + 
  geom_point(size = 0.5, alpha = 0.5) + 
  geom_smooth(method = "lm") +
  geom_text(
    aes(x=9*10^5, y= 0.5), 
...
# Run linear model 
lm_age_29 <- summary(lm(health_welf_bias ~ age_29, data = analysis_df))
...
plot_age_29 <- analysis_df %>%
  ggplot(aes(x = age_29, y = health_welf_bias)) + 
  geom_point(size = 0.5, alpha = 0.5) + 
  geom_smooth(method = "lm") +
  geom_text(
...
# Run linear model 
lm_age_30_64 <- summary(lm(health_welf_bias ~ age_30_64, data = analysis_df))
...
plot_age_30_64 <- analysis_df %>%
  ggplot(aes(x = age_30_64, y = health_welf_bias)) + 
  geom_point(size = 0.5, alpha = 0.5) + 
  geom_smooth(method = "lm") +
  geom_text(
    aes(x=0.5, y= 0.5),     label = paste0(
...
## 65+
# Run linear model 
lm_age_65 <- summary(lm(health_welf_bias ~ age_65 , data = analysis_df))
...
plot_age_65 <- analysis_df %>%
  ggplot(aes(x = age_65, y = health_welf_bias)) + 
  geom_point(size = 0.5, alpha = 0.5) + 
  geom_smooth(method = "lm") +
  geom_text(
    aes(x=0.3, y= 0.5),     
...
grid.arrange(plot_age_29, plot_age_30_64, plot_age_65, nrow=1)
```

```{r pop-density, eval = TRUE}

lm_density <- summary(lm(health_welf_bias ~ density, data = analysis_df))
