...
# TODO Run 2023 from here 
# TODO pull for ministers too, in case we want to add is.same.party 
...
q_parameters <- unique(q_parameters) # Only keep unique MP-date pairs to avoid pulling the same information twice


q_parameters <- q_parameters %>%
  mutate(question_tabled_when = str_extract(question_tabled_when, ".+?(?=T)"))

members <- pull_members("https://members-api.parliament.uk/api/Members", q_parameters)
