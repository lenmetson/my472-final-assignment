...
answering_bodies <- dbGetQuery(db, 
  "
  SELECT question_answering_body, question_answering_body_id
  FROM oral_questions
  ") %>%
  unique()

answering_bodies$is_econ <- NA
answering_bodies$is_health_welf <- NA
...
answering_bodies <- answering_bodies %>%
  ...
  mutate(
    is_econ = 
      ifelse(
        question_answering_body_id %in% c(211, 201, 24, 202, 214), 
        1, 0
      ),
    is_health_welf = 
      ifelse(
        question_answering_body_id %in% c(17, 29), 
        1, 0
      )
  )

dbWriteTable(db, "answering_bodies", answering_bodies, overwrite = TRUE)
...
time_data <- dbGetQuery(db, 
  ...
  SELECT 
    ...
    SUM(answering_bodies.is_econ) AS econ_count,
    SUM(answering_bodies.is_health_welf) AS health_welf_count, 
    COUNT(*) AS total_question_count,
    SUM(answering_bodies.is_econ)/COUNT(*) AS econ_prop,
    SUM(answering_bodies.is_health_welf)/COUNT(*) AS health_welf_prop


  FROM oral_questions
    LEFT JOIN answering_bodies ON oral_questions.question_answering_body_id = answering_bodies.question_answering_body_id
...
geog_data <- dbGetQuery(db, # FIXME: check why filter BETWEEN reduces number returned  
  ...
  FROM oral_questions
    JOIN answering_bodies 
      ON oral_questions.question_answering_body_id = answering_bodies.question_answering_body_id
    JOIN members
...
analysis_df_econ <- dbGetQuery(
  ...
  "
  SELECT 
    answering_bodies.is_econ AS is_econ,
