...
time_health <- time_data_by_party %>% 
...
convert_geojson_to_shapefile <- function(geojson_string, output_path) {
  geojson_data <- st_read(geojson_string, quiet = TRUE)
  st_write(geojson_data, dsn = output_path)
}

#unlink("data/shpfiles",recursive = TRUE) # delete directory 
dir.create("data/shpfiles")

geog_data$geometry <- NA

# Loop through the DataFrame and convert each GeoJSON string to a shapefile
for (i in 1:nrow(geog_data)) {
  geojson_string <- geog_data$con_shapefile[i]
  output_path <- paste0("data/shpfiles/shapefile_", geog_data$constituency[i], ".shp")
  
  convert_geojson_to_shapefile(geojson_string, output_path)

  shapefile_data <- st_read(output_path)
  geog_data$geometry[i] <- shapefile_data[1,2]
  
}

geog_sf$geometry

geo_sf <- st_sfc(lapply(geog_data$geometry, st_multipolygon))
geo_sf <- st_sfc(geog_data$geometry)
geo_sf <- st_union(geog_data$geometry[1:5])

tm_shape(geog_data$geometry) +
  tm_borders() 
...
analysis_df_econ <- dbGetQuery(
  ...
  WHERE 
    parties.party_name IN ('Conservative', 'Labour', 'Liberal Democrat', 'Green Party') /* TODO decide whether to filter by party, or by England and Wales, or keep all in */
  "
  )
