...
CANDO random forests (but won't have words)
...
geog_data <- geog_data %>%
  mutate(health_welf_bias = health_welf_prop - econ_prop)
...
# Plot bias
bias_map <- tm_shape(geog_data) +
  tm_sf(
    col = "health_welf_bias",
    title = "Bias",
   legend.hist = TRUE 
  ) # +
...
tm_shape(geog_data) +
  tm_sf(
    ...
    title = "% Questions: Health & Welfare",
    #legend.hist = TRUE,
    palette = "Reds" 
  ) # +
#  tm_layout(
#   legend.outside = FALSE) # BUG legend not moving outside 
...
analysis_df <- analysis_df %>%
  mutate(health_welf_bias = health_welf_prop - econ_prop)
```
```{r seat-marginality, eval = TRUE}

# Run linear models 
lm_marg_e <- summary(lm(econ_prop ~ mean_marginality, data = analysis_df))

lm_marg_h <- summary(lm(health_welf_prop ~ mean_marginality, data = analysis_df))

# Save slope coefficients and SEs for plotting

slope_marg_e <- lm_marg_e$coefficients[[2,1]]
se_marg_e <- lm_marg_e$coefficients[[2,2]]

slope_marg_h <- lm_marg_h$coefficients[[2,1]]
se_marg_h <- lm_marg_h$coefficients[[2,2]]

# Make plots

econ_marginality <- analysis_df %>%
  ggplot(aes(x=mean_marginality, y = econ_prop)) + 
  geom_point(size = 0.5, alpha = 0.5) + 
  geom_smooth(method = "lm") +
  geom_text(
    aes(x=-0.2, y= 0.5), 
    label = paste0(
      "Slope: ", signif(slope_marg_e, digits = 3),
      "\n SE: ", signif(se_marg_e, digits = 3)
    ),
    size = 3)+  
  ylim(0,1) +
  theme(
...
health_welf_marginality <- analysis_df %>%
  ggplot(aes(x=mean_marginality, y = health_welf_prop)) + 
  geom_point(size = 0.5, alpha = 0.5) + 
  geom_smooth(method = "lm") +
  geom_text(
    aes(x=-0.2, y= 0.5), 
    label = paste0(
      "Slope: ", signif(slope_marg_h, digits = 3),
      "\n SE: ", signif(se_marg_h, digits = 3)
      ),
    size = 3)+
  ylim(0,1) +
  theme(
...
grid.arrange(econ_marginality, health_welf_marginality, nrow=1)

```


```{r con-uc-claimants, eval = TRUE}


# Run linear models 
lm_uc_e <- summary(lm(econ_prop ~ uc_claimants, data = analysis_df))

lm_uc_h <- summary(lm(health_welf_prop ~ uc_claimants, data = analysis_df))

# Save slope coefficients and SEs for plotting

slope_uc_e <- lm_uc_e$coefficients[[2,1]]
se_uc_e <- lm_uc_e$coefficients[[2,2]]

slope_uc_h <- lm_uc_h$coefficients[[2,1]]
se_uc_h <- lm_uc_h$coefficients[[2,2]]

# Make plots

econ_uc <- analysis_df %>%
  ggplot(aes(x=uc_claimants, y = econ_prop)) + 
  geom_point(size = 0.5, alpha = 0.5) + 
  geom_smooth(method = "lm") +
  geom_text(
    aes(x=-0.2, y= 0.5), 
    label = paste0(
      "Slope: ", signif(slope_uc_e, digits = 3),
      "\n SE: ", signif(se_uc_e, digits = 3)
    ),
    size = 3)+  
  ylim(0,1) +
  theme(
...
health_uc <- analysis_df %>%
  ggplot(aes(x=uc_claimants, y = health_welf_prop)) + 
  geom_point(size = 0.5, alpha = 0.5) + 
  geom_smooth(method = "lm") +
  geom_text(
    aes(x=-0.2, y= 0.5), 
    label = paste0(
      "Slope: ", signif(slope_uc_h, digits = 3),
      "\n SE: ", signif(se_uc_h, digits = 3)
      ),
    size = 3)+
  ylim(0,1) +
  theme(
...
grid.arrange(econ_uc, health_uc, nrow=1)

```


```{r con-house-price, eval = TRUE}

# Run linear models 
lm_house_price_e <- summary(lm(econ_prop ~ median_house_price, data = analysis_df))

lm_house_price_h <- summary(lm(health_welf_prop ~ median_house_price, data = analysis_df))

# Save slope coefficients and SEs for plotting

slope_house_price_e <- lm_house_price_e$coefficients[[2,1]]
se_house_price_e <- lm_house_price_e$coefficients[[2,2]]

slope_house_price_h <- lm_house_price_h$coefficients[[2,1]]
se_house_price_h <- lm_house_price_h$coefficients[[2,2]]


# Make plots 

econ_house_price <- analysis_df %>%
  ggplot(aes(x=median_house_price, y = econ_prop)) + 
  geom_point(size = 0.5, alpha = 0.5) + 
  geom_smooth(method = "lm") +
  geom_text(
    aes(x=9*10^5, y= 0.5), 
    label = paste0(
      "Slope: ", signif(slope_house_price_e, digits = 3),
      "\n SE: ", signif(se_house_price_e, digits = 3)
    ),
    size = 3)+  
  ylim(0,1) +
  theme(
...
health_house_price <- analysis_df %>%
  ggplot(aes(x=median_house_price, y = health_welf_prop)) + 
  geom_point(size = 0.5, alpha = 0.5) + 
  geom_smooth(method = "lm") +
  geom_text(
    aes(x=9*10^5, y= 0.5), 
    label = paste0(
      "Slope: ", signif(slope_house_price_h, digits = 3),
      "\n SE: ", signif(se_house_price_h, digits = 3)
      ),
    size = 3)+
  ylim(0,1) +
  theme(
...
grid.arrange(econ_house_price, health_house_price, nrow=1)
...
# Run linear models 
lm_age_29_e <- summary(lm(econ_prop ~ age_29, data = analysis_df))

lm_age_29_h <- summary(lm(health_welf_prop ~ age_29, data = analysis_df))

# Save slope coefficients and SEs for plotting

slope_age_29_e <- lm_age_29_e$coefficients[[2,1]]
se_age_29_e <- lm_age_29_e$coefficients[[2,2]]

slope_age_29_h <- lm_age_29_h$coefficients[[2,1]]
se_age_29_h <- lm_age_29_h$coefficients[[2,2]]


# Make plots 

econ_age_29 <- analysis_df %>%
  ggplot(aes(x = age_29, y = econ_prop)) + 
  geom_point(size = 0.5, alpha = 0.5) + 
  geom_smooth(method = "lm") +
  geom_text(
    aes(x=0.5, y= 0.5),     label = paste0(
      "Slope: ", signif(slope_age_29_e, digits = 3),
      "\n SE: ", signif(se_age_29_e, digits = 3)
    ),
    size = 3)+  
  ylim(0,1) +
  theme(
...
health_age_29 <- analysis_df %>%
  ggplot(aes(x= age_29, y = health_welf_prop)) + 
  geom_point(size = 0.5, alpha = 0.5) + 
  geom_smooth(method = "lm") +
  geom_text(
    aes(x=0.5, y= 0.5), 
    label = paste0(
      "Slope: ", signif(slope_age_29_h, digits = 3),
      "\n SE: ", signif(se_age_29_h, digits = 3)
      ),
    size = 3)+
  ylim(0,1) +
  theme(
...
# Run linear models 
lm_age_30_64_e <- summary(lm(econ_prop ~ age_30_64, data = analysis_df))

lm_age_30_64_h <- summary(lm(health_welf_prop ~ age_30_64, data = analysis_df))

# Save slope coefficients and SEs for plotting

slope_age_30_64_e <- lm_age_30_64_e$coefficients[[2,1]]
se_age_30_64_e <- lm_age_30_64_e$coefficients[[2,2]]

slope_age_30_64_h <- lm_age_30_64_h$coefficients[[2,1]]
se_age_30_64_h <- lm_age_30_64_h$coefficients[[2,2]]


# Make plots 

econ_age_30_64 <- analysis_df %>%
  ggplot(aes(x = age_30_64, y = econ_prop)) + 
  geom_point(size = 0.5, alpha = 0.5) + 
  geom_smooth(method = "lm") +
  geom_text(
    aes(x=0.5, y= 0.5),     label = paste0(
      "Slope: ", signif(slope_age_30_64_e, digits = 3),
      "\n SE: ", signif(se_age_30_64_e, digits = 3)
    ),
    size = 3)+  
  ylim(0,1) +
  theme(
...
health_age_30_64 <- analysis_df %>%
  ggplot(aes(x= age_30_64, y = health_welf_prop)) + 
  geom_point(size = 0.5, alpha = 0.5) + 
  geom_smooth(method = "lm") +
  geom_text(
    aes(x=0.5, y= 0.5), 
    label = paste0(
      "Slope: ", signif(slope_age_30_64_h, digits = 3),
      "\n SE: ", signif(se_age_30_64_h, digits = 3)
      ),
    size = 3)+
  ylim(0,1) +
  theme(
...
# Run linear models 
lm_age_65_e <- summary(lm(econ_prop ~ age_65, data = analysis_df))

lm_age_65_h <- summary(lm(health_welf_prop ~ age_65, data = analysis_df))

# Save slope coefficients and SEs for plotting

slope_age_65_e <- lm_age_65_e$coefficients[[2,1]]
se_age_65_e <- lm_age_65_e$coefficients[[2,2]]

slope_age_65_h <- lm_age_65_h$coefficients[[2,1]]
se_age_65_h <- lm_age_65_h$coefficients[[2,2]]


# Make plots 

econ_age_65 <- analysis_df %>%
  ggplot(aes(x = age_65, y = econ_prop)) + 
  geom_point(size = 0.5, alpha = 0.5) + 
  geom_smooth(method = "lm") +
  geom_text(
    aes(x=0.3, y= 0.5),     label = paste0(
      "Slope: ", signif(slope_age_65_e, digits = 3),
      "\n SE: ", signif(se_age_65_e, digits = 3)
    ),
    size = 3)+  
  ylim(0,1) +
  theme(
...
health_age_65 <- analysis_df %>%
  ggplot(aes(x= age_65, y = health_welf_prop)) + 
  geom_point(size = 0.5, alpha = 0.5) + 
  geom_smooth(method = "lm") +
  geom_text(
    aes(x=0.3, y= 0.5), 
    label = paste0(
      "Slope: ", signif(slope_age_65_h, digits = 3),
      "\n SE: ", signif(se_age_65_h, digits = 3)
      ),
    size = 3)+
  ylim(0,1) +
  theme(
...
grid.arrange(
  econ_age_29, health_age_29, 
  econ_age_30_64, health_age_30_64,  
  econ_age_65, health_age_65,   
  nrow=3)

```

```{r pop-density, eval = TRUE}


lm_density_b <- summary(lm(health_welf_bias ~ density, data = analysis_df))


bias_density <- analysis_df %>%
  ggplot(aes(x=density, y = health_welf_bias)) + 
  geom_point(size = 0.5, alpha = 0.5) + 
  geom_smooth(method = "lm") +
  # geom_text(
  #   aes(x=-0.2, y= 0.5), 
  #   label = paste0(
  #     "Slope: ", signif(slope_density_e, digits = 3),
  #     "\n SE: ", signif(se_density_e, digits = 3)
  #   ),
  #   size = 3)+  
  theme(
    panel.background = element_rect(fill = "white", color = "black"), 
...
# Run linear models 
lm_density_e <- summary(lm(econ_prop ~ density, data = analysis_df))
lm_density_h <- summary(lm(health_welf_prop ~ density, data = analysis_df))

# Save slope coefficients and SEs for plotting

slope_density_e <- lm_density_e$coefficients[[2,1]]
se_density_e <- lm_density_e$coefficients[[2,2]]

slope_density_h <- lm_density_h$coefficients[[2,1]]
se_density_h <- lm_density_h$coefficients[[2,2]]

# Make plots

econ_density <- analysis_df %>%
  ggplot(aes(x=density, y = econ_prop)) + 
  geom_point(size = 0.5, alpha = 0.5) + 
  geom_smooth(method = "lm") +
  geom_text(
    aes(x=-0.2, y= 0.5), 
...
health_welf_density <- analysis_df %>%
  ggplot(aes(x=density, y = health_welf_prop)) + 
  geom_point(size = 0.5, alpha = 0.5) + 
  geom_smooth(method = "lm") +
  geom_text(
    aes(x=-0.2, y= 0.5), 
    label = paste0(
      "Slope: ", signif(slope_density_h, digits = 3),
      "\n SE: ", signif(se_density_h, digits = 3)
      ),
    size = 3)+
  ylim(0,1) +
  theme(
...
grid.arrange(econ_density, health_welf_density, nrow=1)
```
