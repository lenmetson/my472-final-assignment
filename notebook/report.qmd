...
# Before this function there are 4225 observations, and after, only 414.  
...
geog_data <- dbGetQuery(db, # FIXME: check why filter BETWEEN reduces number returned  # Maybe this is exactly what it is SUPPOSED to do?! Like maybe the extra rows are duplicates?? - no because there are no duplicates 
  "
  SELECT 
