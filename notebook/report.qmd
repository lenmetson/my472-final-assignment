...
# TODO add function that converts "NA" to NA, then apply it when reading data in 
...
for (i in seq_along(oral_questions)) { 
  ...
  if (i == 1){
    question_df <- data.frame(oral_questions[[i]])
  } else {
    question_df2 <- data.frame(oral_questions[[i]])
...
for (i in seq_along(written_questions)) { 
  ...
  if (i == 1){
    question_df <- data.frame(written_questions[[i]])
  } else {
    question_df2 <- data.frame(written_questions[[i]])
    question_df <- rbind(question_df, question_df2)
  }
}
...
dbWriteTable(db, "written_questions", question_df, overwrite = TRUE)
...
# TODO see if forests work and repeat for health_welf
```
